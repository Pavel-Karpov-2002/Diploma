...
public class ConstructorTestScript : MonoBehaviour
{
    ...
    private bool IsStudentInformationFilled()
    {
        ...
        {
            GameException.Instance.ShowError("Введите название темы!");
            return false;
        }
        if (amountStudentsOnFloorInput.text == string.Empty)
        {
            GameException.Instance.ShowError("Введите количество студентов на этаже!");
            return false;
        }
        if (amountQuestionsForTestInput.text == string.Empty)
        {
            GameException.Instance.ShowError("Введите количество вопросов у каждого студента!");
            return false;
        }
        if (questions.Count == 0)
        {
            GameException.Instance.ShowError("Создайте хотя бы один вопрос!");
            return false;
        }
        return true;
    }
    ...
    private bool IsTeacherInformationFilled()
    {
        ...
        {
            GameException.Instance.ShowError("Введите количество вопросов в тесте");
            return false;
        }
        if (minPercentCorrectTeacherInput.text == string.Empty)
        {
            GameException.Instance.ShowError("Введите минимальное количество процентов для выполнения теста");
            return false;
        }
        if (minPercentCorrectToUnlockTeacherInput.text == string.Empty)
        {
            GameException.Instance.ShowError("Введите минимальное количество процентов для получения доступа к преподавателю");
            return false;
        }
        if (questions.Count == 0)
        {
    ...
    public void UpdateQuestion(Question question, ChangeQuestionButton button)
    {
        ...
        GameException.Instance.ShowError("Вопрос не был выбран");
    }
    ...
    private bool CheckOnNullParameters()
    {
        ...
        {
            GameException.Instance.ShowError("Введите вопрос");
            return true;
        }

        if (correctAnswerInput.text == string.Empty)
        {
            GameException.Instance.ShowError("Не был введен ответ на вопрос.");
            return true;
        }

        if (questionTimeButton.text == string.Empty)
        {
            GameException.Instance.ShowError("Не было назначено время для ответа на вопрос");
            return true;
        }

        if (pointsForCorrectAnswerInput.text == string.Empty || pointsForWrongAnswerInput.text == string.Empty)
        {
            GameException.Instance.ShowError("Не была назначена стоимость вопроса");
            return true;
        }
        return false;
    }
    ...
    private string[] GetAnswers()
    {
        ...
        for (int i = 0; i < answerInputButtons.Count; i++)
        {
            ...
            {
                GameException.Instance.ShowError("Ответ не был заполнен");
                return null;
            }
        ...
        if (!isCorrectAnswer)
        {
            GameException.Instance.ShowError("Верный ответ не найден в вариантах ответа");
            return null;
        }
        return answers;
    }
